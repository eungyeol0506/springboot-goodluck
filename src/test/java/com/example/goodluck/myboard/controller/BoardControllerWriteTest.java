...
public class BoardControllerWriteTest {
    ...
    class SuccessCase{
        ...
        void successWriteBoard() throws Exception{
            ...
            for(int i=0; i<2; i++){
                MockMultipartFile file = new MockMultipartFile("fileImage" , "test_" + i + ".txt", "image/png", "Hi, Hello".getBytes());
                fileList.add(file);
            }
            ...
            BDDMockito.willDoNothing().given(mockAttachService).uploadAttachList(any(MyBoard.class), anyList());
            
            MultiValueMap<String, String> formData = new LinkedMultiValueMap();
            formData.add("boardTitle", "Test");
            formData.add("contents", "Test");
            // when & then
            mockMvc.perform(MockMvcRequestBuilders.multipart("/board/write")
                                                  .file(fileList.get(0))
                                                  .file(fileList.get(1))
                                                  .session(session)
                                                  .params(formData)
